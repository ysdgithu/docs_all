...
<td>Number</td>
<td>Yes</td>
<td>The sequence of the message. Method for obtaining `MsgSeq`: - If a one-to-one message is sent from the **client**, obtain `MsgSeq` by using the [Callback on message sent](../callbacks/message-sent.mdx) operation. - If a one-to-one message is sent by using the server API [`SendPeerMessage`](./send-a-one-to-one-message.mdx), obtain `MsgSeq` in the response data.</td>
</tr>
<tr>
<td>Payload</td>
<td>String</td>
...
<td>Object</td>
<td>No</td>
<td>The offline push configuration. For more information, see [OfflinePush Description](./messagebody-introduction.mdx#offlinepush-introduction-inapplicable-for-room-messages).</td>
</tr>
</tbody></table>
