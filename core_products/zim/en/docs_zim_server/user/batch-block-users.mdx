...
<td>
The return code.
<Note title="Note">When you request to block multiple users at a time, take note of the following items:<ul><li>If one or more users are successfully blocked, `0` is returned. For more information, see `ErrList`.</li><li>If all specified users fail to be blocked, a corresponding code is returned. For more information, see [Return codes](../return-codes.mdx).</li></ul></Note>
</td>
</tr>
<tr data-row-level="3">
<td>Message</td>
